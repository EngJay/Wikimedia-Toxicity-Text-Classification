...
def prepare_data(raw_data):
    ...
    #     le = LabelEncoder()
    #     y.append(le.fit_transform(doc_labels))
    ...
    # one_hot_labels = np.zeros((num_docs, len(labels))).astype(int)
    #
...
def parser(x, length, y):
    # TODO Add IDs so examples can be identified.
...
def train_input_fn(params):
    dataset = tf.data.Dataset.from_tensor_slices((x_train,
                                                  x_len_train,
                                                  y_train))
    # TODO Set shuffle based on size of dataset?
...
def cnn_model_fn(features, labels, mode, params):
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
        ...
        one_hot_predictions = multi_label_hot(predictions)
    ...
    # Compute the loss.
    # sigmoid_cross_entropy_with_logits() computes the sigmoid and the loss, so
    ...
    if mode == tf.estimator.ModeKeys.EVAL:
        ...
        if FLAGS.use_tpu:
            ...
            return tf.contrib.tpu.TPUEstimatorSpec(
                mode=mode,
                loss=loss,
                eval_metrics=(metric_fn, [labels, one_hot_predictions])
            )
        else:
            # Not on TPU, so use the standard stuff.
            metrics_dict = {
...
def main(argv):
    ...
    with open(str(csv_path), mode='a') as csv_file:
        ...
        # Write header if results csv did not exist at start of this trip through.
    ...
    if FLAGS.enable_predict:
        ...
        for pred_dict in predictions:
            ...
            print(template.format(class_id, probability))
