...
# vocab = np.concatenate((vocab, np.random.rand(1, vocab.shnano w   ape[1])))
...
def cnn_model_fn(features, labels, mode, params):
    ...
    if mode == tf.estimator.ModeKeys.EVAL:
        ...
        def metric_fn(labels, logits):
            ...
            #cnf_matrix = tf.math.confusion_matrix(labels=labels,
            #                                       predictions=tf.argmax(logits,
            #                                                             axis=1),
            #                                       name="confusion_matrix")
            # # Coupled to number of labels.
            # tp = cnf_matrix[1, 1].eval()
            #
            # tn = cnf_matrix[0, 0].eval()
            #
            # fp = cnf_matrix[1, 0].eval()
            #
            # fn = cnf_matrix[0, 1].eval()
...
def main(argv):
    ...
    def word_embedding_initializer(shape=None, dtype=tf.float32,
                                   partition_info=None):
