...
def create_model():
    # TODO Multichannel CNN example:
    # https://medium.com/@dsouza.amanda/multi-channel-cnn-for-text-699713aa98a7
    # Two embedding input layers, then they're merged and reshaped.
    ...
    word_embedding_input = tf.keras.models.Sequential()
    word_embedding_input.add(
        tf.keras.layers.Embedding(vocab_size,
                                  word_embedding_size,
                                  weights=[word_embedding_matrix],
                                  input_length=FLAGS.max_doc_length,
                                  mask_zero=True,
                                  trainable=False)
    )

    # Lexicon embeddings input layer.
    # TODO Pretty sure this will need to be padded to match word embeddings size.
    lex_size = len(lex_emb)
    lex_embedding_matrix = lex_emb.astype(np.float32)
    lex_embedding_size = lex_embedding_matrix.shape[1]

    # Word embeddings should be wider than lexicons.
    embeddings_delta = word_embedding_size - lex_embedding_size
    padded_lex_embedding_matrix = tf.keras.layers.ZeroPadding2D(
        ((0, 0), (0, embeddings_delta))
    )

    lex_embedding_input = tf.keras.models.Sequential()
    lex_embedding_input.add(
        tf.keras.layers.Embedding(lex_size,
                                  padded_lex_embedding_matrix,
                                  weights=[lex_embedding_matrix],
                                  input_length=FLAGS.max_doc_length,
                                  mask_zero=True,
                                  trainable=True)
    )
    ...
    model.add(tf.keras.layers.Concatenate([word_embedding_input,
                                           lex_embedding_input], axis=-1))

    model.add(tf.keras.layers.Reshape(
        (2, FLAGS.max_doc_length, word_embedding_size))
    )

    model.add(
        tf.layers.Conv2D(64, 3, word_embedding_size, activation="relu",
                         padding="same"))
    model.add(tf.layers.MaxPooling2D((3, 3), (FLAGS.max_doc_length - 3 + 1, 1)))

    model.add(tf.layers.Flatten())

    model.add(tf.layers.Dense(256, activation="relu"))

    #model.add(tf.layers.Dense(labels_size, activation="softmax"))
...
def cnn_model_fn(features, labels, mode, params):
    ...
    # Pad lexicon layer to match embedding layer.
    # lex_embedding_input_layer

    # Concatenate lexicon data to word embeddings.
    # doc_embed = tf.concat([word_embedding_input_layer,
    #                        lex_embedding_input_layer], axis=-1)
    ...
    # # strides,
    # # padding,
    # conv2d3 = tf.layers.conv2d(doc_embed, 64, 3, padding="same",
    #                            activation=tf.nn.relu)
    # # pool_size,
    # # strides,
    # # padding = 'valid',
    # # data_format = 'channels_last',
    # pool3 = tf.layers.max_pooling2d(conv2d3, (FLAGS.max_doc_length - 3 + 1))
    #
    # conv2d4 = tf.layers.conv2d(doc_embed, 64, 4, padding="same",
    #                            activation=tf.nn.relu)
    # # pool_size,
    # # strides,
    # # padding = 'valid',
    # # data_format = 'channels_last',
    # pool4 = tf.layers.max_pooling2d(conv2d4, (FLAGS.max_doc_length - 4 + 1))
    #
    # conv2d5 = tf.layers.conv2d(doc_embed, 64, 5, padding="same",
    #                            activation=tf.nn.relu)
    # # pool_size,
    # # strides,
    # # padding = 'valid',
    # # data_format = 'channels_last',
    # pool5 = tf.layers.max_pooling2d(conv2d5, (FLAGS.max_doc_length - 5 + 1))
    #
