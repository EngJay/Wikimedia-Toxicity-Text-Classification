...
def main(argv):
    ...
    try:
        ...
        print('data_prep_wikimedia_attacks.py -m <min_num_votes> -d <data_path>')
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print('data_prep_wikimedia_attacks.py -n <num_records>\n'
                  + 'Defaults:\n'
    ...
    date_filename = 'WM-PA-Binary-Data-min-' + str(min_num_votes) + '-votes.bin'
    ...
    # TODO Take arg for size of embeddings.
    embeddings_path = output_dir / 'WM-PA-Gao-200-embeddings'
    np.save(embeddings_path, vocab)

    id2word_path = output_dir / 'WM-PA-Gao-id2word.bin'
