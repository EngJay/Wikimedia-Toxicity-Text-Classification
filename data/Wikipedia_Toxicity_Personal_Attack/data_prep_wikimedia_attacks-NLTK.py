...
def main(argv):
    ...
    try:
        ...
        print('train.py -m <min_num_votes> -d <data_path>')
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
    ...
    with open(csv_path, mode='r') as csv_file:
        ...
        for line in csv_reader:
            # Skip header.
            lineno += 1
            sys.stdout.write('Pocessing line %i     \r' % lineno)
            sys.stdout.flush()
    ...
    date_filename = 'PA-NLTK-min-' + str(min_num_votes) + '-votes-data.bin'
    ...
    # TODO Take arg for size of embeddings.
    embeddings_path = output_dir / 'NLTK-PA-300-embeddings'
    np.save(embeddings_path, vocab)

    id2word_path = output_dir / 'NLTK-PA-300-id2word.bin'
    with open(id2word_path, 'wb') as f:
        msgpack.pack(id2word, f)
        
    word2id_path = output_dir / 'NLTK-PA-300-word2id.bin'
    with open(word2id_path, 'wb') as f:
        msgpack.pack(word2id, f)
