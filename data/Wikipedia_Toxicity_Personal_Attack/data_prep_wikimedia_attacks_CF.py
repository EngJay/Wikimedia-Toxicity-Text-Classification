...
def main(argv):
    ...
    for opt, arg in opts:
        if opt == '-h':
            print('data_prep_wikimedia_attacks-GN.py -n\n'
                  + 'Defaults:\n'
...
#     unk = len(vocab) - 1
    ...
    f = BytesIO(file_io.read_file_to_string('../Post-Specialized-Embeddings/counter-fitting/results/Gao_PA_CF_word2id.bin',
                                            binary_mode=True))
    ...
    date_filename = 'wikimedia-PA-CF-min-' + str(min_num_votes) + '-votes-data.bin'
