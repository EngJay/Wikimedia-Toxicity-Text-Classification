...
def main(argv):
    ...
    try:
        ...
        print('data_prep_wikimedia_attacks_Gao_AR.py -m <min_num_votes> -d <data_path>')
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print('data_prep_wikimedia_attacks_Gao_AR.py\n'
                  + 'Defaults:\n'
...
#     unk = len(vocab) - 1
    ...
    f = BytesIO(file_io.read_file_to_string('../Post-Specialized-Embeddings/attract-repel/results/Gao_PA_AR_word2id.bin',
                                            binary_mode=True))
    ...
    date_filename = 'wikimedia-personal-attacks-min-' + str(min_num_votes) + '-votes-Gao-AR-data.bin'
